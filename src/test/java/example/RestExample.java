...
import static com.svedentsov.rest.helper.RestMatcher.body;
...
public class RestExample {
    ...
    public void validateMyEntity(Response response) {
        ...
        EntityValidator.of(entity).shouldHave(
                value(MyEntity::getName, startsWith("Ent")), // name начинается с "Ent"
                value(MyEntity::getStatus, isUpperCase()), // status — только заглавные буквы
                value(MyEntity::getAge, numberEqualTo(30)), // age == 30
                value(MyEntity::getDescription, contains("Sample")) // description содержит слово "Sample"
        );
    }
    ...
    public void validateMyEntityList(Response response) {
        ...
        for (MyEntity e : entities) {
            EntityValidator.of(e).shouldHave(
                    value(MyEntity::getDescription, hasNonBlankContent())
            );
        }
    }
}
